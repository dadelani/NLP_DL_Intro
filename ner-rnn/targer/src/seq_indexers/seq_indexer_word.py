...
class SeqIndexerWord(SeqIndexerBaseEmbeddings):
    ...
    def load_items_from_embeddings_file_and_unique_words_list(self, emb_fn, emb_delimiter, emb_load_all,
                                                              unique_words_list):
        ## CHANGED CODE HERE TO LOAD FROM GENSIM ##
        embeddings_model = KeyedVectors.load(emb_fn, mmap='r') #FastText.load_fasttext_format(emb_fn)  #KeyedVectors.load(emb_fn, mmap='r')
        out_of_vocabulary_words_list = list()

        for unique_word in unique_words_list:
            emb_vector = self.get_embedding_vector_for_word(unique_word, embeddings_model)
            if emb_vector is None:
                out_of_vocabulary_words_list.append(unique_word)
            else:
                self.add_word_emb_vec(unique_word, emb_vector)
    ...
    def get_embedding_vector_for_word(self, word, embeddings_model):
        if word in embeddings_model:
            self.original_words_num += 1
            return embeddings_model[word]
        elif self.check_for_lowercase and word.lower() in embeddings_model:
            self.lowercase_words_num += 1
            return embeddings_model[word.lower()]
        elif self.zero_digits and re.sub('\d', '0', word) in embeddings_model:
            self.zero_digits_replaced_num += 1
            return embeddings_model[re.sub('\d', '0', word)]
        elif self.check_for_lowercase and self.zero_digits and re.sub('\d', '0', word.lower()) in embeddings_model:
            self.zero_digits_replaced_lowercase_num += 1
            return embeddings_model[re.sub('\d', '0', word.lower())]
        return None
