...
class DatasetsBank():
    ...
    def add_train_noisy_sequences(self, word_sequences_train_noisy, tag_sequences_train_noisy, noisy_subsample_factor):
        self.train_noisy_data_num = len(word_sequences_train_noisy)
        self.word_sequences_train_noisy = word_sequences_train_noisy
        self.tag_sequences_train_noisy = tag_sequences_train_noisy
        self.__add_to_unique_words_list(word_sequences_train_noisy)
        self.noisy_subsample_factor = noisy_subsample_factor
    ...
    def __get_train_noisy_batch(self, batch_indices):
        word_sequences_train_noisy_batch = [self.word_sequences_train_noisy[i] for i in batch_indices]
        tag_sequences_train_noisy_batch = [self.tag_sequences_train_noisy[i] for i in batch_indices]
        return word_sequences_train_noisy_batch, tag_sequences_train_noisy_batch
        
    def get_train_noisy_batches(self, batch_size):
        noisy_subsample_size = int(self.train_data_num) * self.noisy_subsample_factor
        random_indices = np.random.permutation(np.arange(self.train_data_num))
        random_indices = random_indices[:noisy_subsample_size]
        for k in range(len(random_indices) // batch_size): # oh yes, we drop the last batch
            batch_indices = random_indices[k:k + batch_size].tolist()
            word_sequences_train_noisy_batch, tag_sequences_train_noisy_batch = self.__get_train_noisy_batch(batch_indices)
            yield word_sequences_train_noisy_batch, tag_sequences_train_noisy_batch
